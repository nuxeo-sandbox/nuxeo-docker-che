...
EOF
  ...
  gosu $NUXEO_USER $NUXEO_CTL mp-init
  if [ -n "$NUXEO_CLID"  ] && [ ${NUXEO_INSTALL_HOTFIX:='true'} == "true" ]; then
      gosu $NUXEO_USER $NUXEO_CTL mp-hotfix --accept=true
  fi

  # Install packages if exist
  if [ -n "$NUXEO_PACKAGES" ]; then
    gosu $NUXEO_USER $NUXEO_CTL mp-install $NUXEO_PACKAGES --relax=false --accept=true
  fi
  ...
  shift
  exec gosu $NUXEO_USER $NUXEO_CTL $@
fi
